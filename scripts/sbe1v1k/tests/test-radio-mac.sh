...
if grep -Eiq 'band|freq_range|path_match|phy_path|resolve_sysfs' "$radio_mac"; then
	bad "radio-mac.uc must not grow path/band fallbacks"
else
	ok "radio-mac.uc has no path/band fallbacks"
fi
