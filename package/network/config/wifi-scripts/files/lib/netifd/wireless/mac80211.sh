...
# hostapd config_set may defer until prior radios leave ACS/DFS — long timeout only here.
ubus_call_hostapd_config_set() {
	flock /var/run/hostapd.lock ubus -t 300 call hostapd config_set "$@"
}
...
# Multi-radio hostapd serialization lives in hostapd.uc (phy-setup-queue).
# wifi-scripts must not mkdir-lock around config_set.
...
hostapd_set_config() {
	...
	[ -n "$hostapd_ctrl" ] || {
		ubus_call_hostapd_config_set '{ "phy": "'"$phy"'", "radio": '"$radio"', "config": "", "prev_config": "'"${hostapd_conf_file}.prev"'" }' > /dev/null
		return 0;
	}

	ubus wait_for hostapd
	local hostapd_res="$(ubus_call_hostapd_config_set "{ \"phy\": \"$phy\", \"radio\": $radio, \"config\":\"${hostapd_conf_file}\", \"prev_config\": \"${hostapd_conf_file}.prev\"}")"
	ret="$?"
...
mac80211_reset_config() {
	...
	ubus_call_hostapd_config_set '{ "phy": "'"$phy"'", "radio": '"$radio"', "config": "", "prev_config": "'"$hostapd_conf_file"'" }' > /dev/null
